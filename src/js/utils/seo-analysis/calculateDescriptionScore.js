import {
  callToActionKeywords, stopWords,
  WEIGHTS
} from "./const";
...
function calculateCallToActionScore(description) {
  const descriptionWords = description.split(" ");
  const callToActionCount = descriptionWords.filter((word) =>
    callToActionKeywords.includes(word.toLowerCase())
  ).length;

  let score = parseInt((callToActionCount / descriptionWords.length) * 100);
...
function calculateUniquenessScore(title, previousTitles = []) {
  ...
  if (isDuplicate) {
    return {
      score: 0,
      message:
        "Mô tả đã được sử dụng trước đó, cần thay đổi để đạt yếu tố uniqueness",
    };
  } else if (score < 20) {
    return {
    ...
    return {
      score: score.toFixed(2),
      message: `Mô tả đạt yếu tố uniqueness cần cải thiện (${score.toFixed(
        2
      )} điểm)`,
    };
  } else if (score >= 40 && score < 60) {
    return {
      score: score.toFixed(2),
      message: `Mô tả đạt yếu tố uniqueness ở mức trung bình (${score.toFixed(
        2
      )} điểm)`,
    };
  } else if (score >= 60 && score < 80) {
    ...
    return {
      score: score.toFixed(2),
      message: `Mô tả đạt yếu tố uniqueness tuyệt vời (${score.toFixed(
        2
      )} điểm)`,
    };
  }
...
function calculateReadabilityScore(title) {
  ...
  if (score >= 90) {
    message +=
      "Mô tả có độ khó rất thấp, phù hợp cho độc giả từ lớp 1 trở lên.";
  } else if (score >= 80) {
    message += "Mô tả có độ khó thấp, phù hợp cho độc giả từ lớp 3 trở lên.";
  } else if (score >= 70) {
    message +=
      "Mô tả có độ khó trung bình, phù hợp cho độc giả từ lớp 5 trở lên.";
  } else if (score >= 60) {
    ...
    message +=
      "Mô tả có độ khó cực cao, chỉ phù hợp cho độc giả có trình độ cao.";
  }

...
function extractHtml(html) {
    ...
    tags.forEach(function (tag) {
        ...
        for (const match of matches) {
            ...
            let newHeadingHtml = oldHeadingHtml.replace(`<${tag}`, `<${tag} id="${slug}"`)
...
function toNested(allHeading) {
    ...
    reverseTags.forEach(function (tag, index) {
        ...
        let childHeadings = sortByIndex(allHeading.filter((x) => x.tag === tag))
        let parentHeadings = allHeading.filter((x) => x.tag === parentTag)

        for (const heading of childHeadings) {
            let parent = parentHeadings.find((x, index) => x.index < heading.index)
            if (parent) {
